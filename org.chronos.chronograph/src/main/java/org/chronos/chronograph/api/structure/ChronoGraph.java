...
public interface ChronoGraph extends Graph {
	 ...
	 *
	 * <p>
	 * This is similar to {@link #configuration()}, except that this method returns an object that provides handy accessor methods for the underlying data.
	 *
	 ...
	 *
	 * <p>
	 * After closing a graph, no further data can be retrieved from or stored in it. Calling this method on an already closed graph has no effect.
	 */
	@Override
	 ...
	 *
	 * <p>
	 * If this method returns <code>true</code>, data access methods on the graph will throw exceptions when attempting to execute them.
	 *
	 ...
	 *
	 * @param branchName
	 *            The name of the branch to retrieve the "now" timestamp for. Must refer to an existing branch. Must not be <code>null</code>.
	 *
	 * @return The "now" timestamp on the given branch. If no commits have occurred on the branch yet, this method returns zero (master branch) or the branching timestamp (non-master branch), otherwise a positive value.
	 */
	 ...
	 *
	 * <p>
	 * Each returned timestamp reflects a point in time when a commit occurred that changed the vertex in question. The same commit may have simultaneously also changed other elements in the graph. Opening a transaction on this timestamp, and retrieving the vertex by id from it will produce the vertex in the state at that point in time.
	 *
	 * <p>
	 * <b>NOTE:</b> This method requires an open transaction! The returned history will always be the history up to and including (but not after) the transaction timestamp!
	 *
	 * @param vertexId
	 ...
	 *
	 * <p>
	 * Each returned timestamp reflects a point in time when a commit occurred that changed the edge in question. The same commit may have simultaneously also changed other elements in the graph. Opening a transaction on this timestamp, and retrieving the edge by id from it will produce the vertex in the state at that point in time.
	 *
	 * <p>
	 * <b>NOTE:</b> This method requires an open transaction! The returned history will always be the history up to and including (but not after) the transaction timestamp!
	 *
	 ...
	 *
	 * <p>
	 * Each returned timestamp reflects a point in time when a commit occurred that changed the edge in question. The same commit may have simultaneously also changed other elements in the graph. Opening a transaction on this timestamp, and retrieving the edge by id from it will produce the edge in the state at that point in time.
	 *
	 * <p>
	 * <b>NOTE:</b> This method requires an open transaction! The returned history will always be the history up to and including (but not after) the transaction timestamp!
	 *
	 ...
	 *            The lower bound of the time range to search in. Must not be negative. Must be less than or equal to <code>timestampUpperBound</code>. Must be less than or equal to the transaction timestamp.
	 * @param timestampUpperBound
	 *            The upper bound of the time range to search in. Must not be negative. Must be greater than or equal to <code>timestampLowerBound</code>. Must be less than or equal to the transaction timestamp.
	 *
	 * @return An iterator over pairs, containing the change timestamp at the first and the modified vertex id at the second position. May be empty, but never <code>null</code>.
	 */
	 ...
	 *            The upper bound of the time range to search in. Must not be negative. Must be greater than or equal to <code>timestampLowerBound</code>. Must be less than or equal to the transaction timestamp.
	 *
	 * @return An iterator over pairs, containing the change timestamp at the first and the modified edge id at the second position. May be empty, but never <code>null</code>.
	 */
	public Iterator<Pair<Long, String>> getEdgeModificationsBetween(final long timestampLowerBound,
			final long timestampUpperBound);

	/**
	 * Returns the metadata for the commit on the {@linkplain ChronoDBConstants#MASTER_BRANCH_IDENTIFIER master} branch at the given timestamp.
	 *
	 * <p>
	 * This search will include origin branches (recursively), if the timestamp is before the branching timestamp.
	 *
	 * @param timestamp
	 *            The timestamp to get the commit metadata for. Must match the commit timestamp exactly. Must not be negative.
	 *
	 * @return The commit metadata. May be <code>null</code> if there was no metadata for the commit, or there has not been a commit at the specified branch and timestamp.
	 */
	...
	/**
	 * Returns an iterator over all timestamps where commits have occurred on the {@linkplain ChronoDBConstants#MASTER_BRANCH_IDENTIFIER master} branch, bounded between <code>from</code> and <code>to</code>, in descending order.
	 *
	 * <p>
	...
	/**
	 * Returns an iterator over all timestamps where commits have occurred on the given branch, bounded between <code>from</code> and <code>to</code>, in descending order.
	 *
	 * <p>
	 * If the <code>from</code> value is greater than the <code>to</code> value, this method always returns an empty iterator.
	 *
	...
	/**
	 * Returns an iterator over all timestamps where commits have occurred on the {@linkplain ChronoDBConstants#MASTER_BRANCH_IDENTIFIER master} branch, bounded between <code>from</code> and <code>to</code>.
	 *
	 * <p>
	 * If the <code>from</code> value is greater than the <code>to</code> value, this method always returns an empty iterator.
	 *
	...
	/**
	 * Returns an iterator over the entries of commit timestamp and associated metadata on the {@linkplain ChronoDBConstants#MASTER_BRANCH_IDENTIFIER master} branch, bounded between <code>from</code> and <code>to</code>, in descending order.
	 *
	 * <p>
	 * If the <code>from</code> value is greater than the <code>to</code> value, this method always returns an empty iterator.
	 *
	 ...
	 *            The upper bound of the time range to look for commits in (inclusive). Must not be negative. Must be less than or equal to the timestamp of this transaction.
	 *
	 * @return An iterator over the commits in the given time range in descending order. The contained entries have the timestamp as the {@linkplain Entry#getKey() key} component and the associated metadata as their {@linkplain Entry#getValue() value} component (which may be <code>null</code>). May be empty, but never <code>null</code>.
	 */
	...
	/**
	 * Returns an iterator over the entries of commit timestamp and associated metadata on the {@linkplain ChronoDBConstants#MASTER_BRANCH_IDENTIFIER master} branch, bounded between <code>from</code> and <code>to</code>.
	 *
	 * <p>
	 * If the <code>from</code> value is greater than the <code>to</code> value, this method always returns an empty iterator.
	 *
	...
	/**
	 * Returns an iterator over commit timestamps on the {@linkplain ChronoDBConstants#MASTER_BRANCH_IDENTIFIER master} branch in a paged fashion.
	 *
	 * <p>
	...
	/**
	 * Returns an iterator over commit timestamps and associated metadata on the {@linkplain ChronoDBConstants#MASTER_BRANCH_IDENTIFIER master} branch in a paged fashion.
	 *
	 * <p>
	 * For example, calling {@code getCommitTimestampsPaged(10000, 100, 0, Order.DESCENDING)} will give the latest 100 commit timestamps that have occurred before timestamp 10000. Calling {@code getCommitTimestampsPaged(123456, 200, 2, Order.DESCENDING} will return 200 commit timestamps, skipping the 400 latest commit timestamps, which are smaller than 123456.
	 *
	...
	/**
	 * Returns pairs of commit timestamp and commit metadata which are "around" the given timestamp on the time axis and on the {@linkplain ChronoDBConstants#MASTER_BRANCH_IDENTIFIER master} branch.
	 *
	 * <p>
	 ...
	 *
	 * @return A list of pairs. The keys are commit timsetamps, the corresponding values are the commit metadata objects (which may be <code>null</code>). The list itself will never be <code>null</code>, but may be empty (if there are no commits to report). The list is sorted in descending order by timestamps.
	 */
	 ...
	 *
	 * @param branch
	 *            The name of the branch to execute the search on. Must refer to an existing branch, and must in particular not be <code>null</code>.
	 * @param timestamp
	 *            The request timestamp around which the commits should be centered. Must not be negative.
	 * @param count
	 *            How many commits to retrieve around the request timestamp. By default, the closest <code>count/2</code> commits will be taken on both sides of the request timestamp. Must not be negative.
	...
	/**
	 * Returns pairs of commit timestamp and commit metadata which are strictly before the given timestamp on the time axis and on the {@linkplain ChronoDBConstants#MASTER_BRANCH_IDENTIFIER master} branch.
	 *
	 * <p>
	 * For example, calling {@link #getCommitMetadataBefore(long, int)} with a timestamp and a count of 10, this method will return the latest 10 commits (strictly) before the given request timestamp.
	 *
	 * @param timestamp
	 ...
	 *
	 * <p>
	 * For example, calling {@link #getCommitMetadataBefore(String, long, int)} with a timestamp and a count of 10, this method will return the latest 10 commits (strictly) before the given request timestamp.
	 *
	 * @param branch
	 *            The name of the branch to execute the search on. Must refer to an existing branch, and must in particular not be <code>null</code>.
	 * @param timestamp
	...
	/**
	 * Returns pairs of commit timestamp and commit metadata which are strictly after the given timestamp on the time axis and on the {@linkplain ChronoDBConstants#MASTER_BRANCH_IDENTIFIER master} branch.
	 *
	 * <p>
	 * For example, calling {@link #getCommitMetadataAfter(long, int)} with a timestamp and a count of 10, this method will return the oldest 10 commits (strictly) after the given request timestamp.
	 *
	 * @param timestamp
	 *            The timestamp to investigate. Must not be negative.
	 * @param count
	 ...
	 *
	 * <p>
	 * For example, calling {@link #getCommitMetadataAfter(String, long, int)} with a timestamp and a count of 10, this method will return the oldest 10 commits (strictly) after the given request timestamp.
	 *
	 * @param branch
	 *            The name of the branch to execute the search on. Must refer to an existing branch, and must in particular not be <code>null</code>.
	 * @param timestamp
	...
	/**
	 * Returns a list of commit timestamp which are "around" the given timestamp on the time axis and on the {@linkplain ChronoDBConstants#MASTER_BRANCH_IDENTIFIER master} branch.
	 *
	 * <p>
	 * By default, this method will attempt to return the closest <code>count/2</code> commits before and after the given timestamp. However, if there are not enough elements on either side, the other side will have more entries in the result list (e.g. if the request count is 10 and there are only two commits before the request timestamp, the list of commits after the request timestamp will have 8 entries instead of 5 to create a list of total length 10). In other words, the result list will always have as many entries as the request <code>count</code>, except when there are not as many commits on the store yet.
	 *
	...
	/**
	 * Returns a list of commit timestamps which are strictly before the given timestamp on the time axis and on the {@linkplain ChronoDBConstants#MASTER_BRANCH_IDENTIFIER master} branch.
	 *
	 * <p>
	 * For example, calling {@link #getCommitTimestampsBefore(long, int)} with a timestamp and a count of 10, this method will return the latest 10 commits (strictly) before the given request timestamp.
	 *
	 * @param timestamp
	 *            The timestamp to investigate. Must not be negative.
	 * @param count
	 ...
	 *
	 * <p>
	 * For example, calling {@link #getCommitTimestampsBefore(String, long, int)} with a timestamp and a count of 10, this method will return the latest 10 commits (strictly) before the given request timestamp.
	 *
	 * @param branch
	 *            The name of the branch to execute the search on. Must refer to an existing branch, and must in particular not be <code>null</code>.
	 * @param timestamp
	...
	/**
	 * Returns a list of commit timestamps which are strictly after the given timestamp on the time axis and on the {@linkplain ChronoDBConstants#MASTER_BRANCH_IDENTIFIER master} branch.
	 *
	 * <p>
	 * For example, calling {@link #getCommitTimestampsAfter(long, int)} with a timestamp and a count of 10, this method will return the oldest 10 commits (strictly) after the given request timestamp.
	 *
	 * @param timestamp
	 *            The timestamp to investigate. Must not be negative.
	 * @param count
	 ...
	 *
	 * <p>
	 * For example, calling {@link #getCommitTimestampsAfter(String, long, int)} with a timestamp and a count of 10, this method will return the oldest 10 commits (strictly) after the given request timestamp.
	 *
	 * @param branch
	 *            The name of the branch to execute the search on. Must refer to an existing branch, and must in particular not be <code>null</code>.
	 * @param timestamp
	...
	/**
	 * Counts the number of commit timestamps on the {@link ChronoDBConstants#MASTER_BRANCH_IDENTIFIER master} branch between <code>from</code> (inclusive) and <code>to</code> (inclusive).
	 *
	 * <p>
	...
	/**
	 * Counts the total number of commit timestamps on the {@link ChronoDBConstants#MASTER_BRANCH_IDENTIFIER master} branch.
	 *
