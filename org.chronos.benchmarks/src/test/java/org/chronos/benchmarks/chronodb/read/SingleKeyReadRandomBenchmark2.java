 ...
 * read calls are executed for this very key. This is done at three revisions: the oldest, the head revision and a
 * random revision. This forms a list of samples, where each sample reflects how long it took to read the random
 * revision, based on the number of versions on this key.
 *
 * <p>
...
public class SingleKeyReadRandomBenchmark2 extends AllChronoDBBackendsTest {
	...
	private void writeVersionTimesXreadHeadTimesY(final int writes, final int reads, final int sampleEveryXWrites) {
		...
		logInfo("");
		logInfo("Iteration\t\tType\t\tAverage\t\tStandard Deviation");
		for (int iteration = 0; iteration < writes; iteration++) {
			...
			if (iteration % sampleEveryXWrites == 0) {
				{ // read base
					...
					logInfo(iteration + "\t\tBase\t\t" + avg + "\t\t" + std);
				}
				{ // read random
					TimeStatistics statistics = Measure.multipleTimes(500, false, () -> {
						for (int read = 0; read < reads; read++) {
							long timestamp = (long) Math.floor(Math.floor(Math.random() * (maxTS - minTS)) + minTS);
							@SuppressWarnings("unused")
							Double readValue = db.tx(timestamp).get(key);
						}
					});
					double avg = statistics.getAverageRuntime();
					double std = statistics.getStandardDeviation();
					logInfo(iteration + "\t\tRandom\t\t" + avg + "\t\t" + std);
				}
				{ // read head
					TimeStatistics statistics = Measure.multipleTimes(500, false, () -> {
						for (int read = 0; read < reads; read++) {
							long timestamp = maxTS;
							@SuppressWarnings("unused")
							Double readValue = db.tx(timestamp).get(key);
						}
					});
					double avg = statistics.getAverageRuntime();
					double std = statistics.getStandardDeviation();
					logInfo(iteration + "\t\tHead\t\t" + avg + "\t\t" + std);
				}
			}
		}
		logInfo("");
