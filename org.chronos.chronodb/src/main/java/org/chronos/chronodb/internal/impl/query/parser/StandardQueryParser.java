 ...
 *
 * <p>
 * This class operates on a {@link StandardQueryTokenStream} that is passed to the constructor. Instances of this class
 * can be created as needed by using that constructor.
 *
 * <p>
...
public class StandardQueryParser implements QueryParser {
	 ...
	 *
	 * @param tokenStream
	 *            The token stream to parse. Must not be <code>null</code>. Must not be in use by another parser.
	 *
	...
	/**
	 * Parses the root-level <i>query</i> in the query language from the token stream.
	 *
	 * <p>
	...
	private QueryElement parseExpression(final QueryTokenStream tokenStream) throws ChronoDBQuerySyntaxException {
		...
		if (orToken != null) {
			...
			QueryElement secondTerm = this.parseTerm(tokenStream);
	...
	private QueryElement parseTerm(final QueryTokenStream tokenStream) throws ChronoDBQuerySyntaxException {
		...
		if (andToken != null) {
			// we have the AND case, parse the second factor
			QueryElement secondFactor = this.parseFactor(tokenStream);
			// construct the binary operation element
	...
	private QueryElement parseFactor(final QueryTokenStream tokenStream) throws ChronoDBQuerySyntaxException {
		...
		if (whereToken != null) {
			...
			String indexName = whereToken.getIndexName();
			Condition condition = whereToken.getCondition();
			TextMatchMode matchMode = whereToken.getMatchMode();
			String comparisonValue = whereToken.getComparisonValue();
			QueryElement whereElement = new WhereElement(indexName, condition, matchMode, comparisonValue);
			return whereElement;
		}
	...
	/**
	 * Attempts to match the next token in the stream with the given token class.
	 *
	 * <p>
	 * If the next token in the stream is an instance of the given expected token class, the token is consumed from the
	 * stream and returned. Otherwise, the token remains in the stream and this method returns <code>null</code>.
	 *
	 * @param <T>
	 *            The expected type of token which should come next in the stream.
	 * @param tokenStream
	 *            The token stream to parse. Must not be <code>null</code>. Must not be in use by another parser.
	 * @param expectedTokenClass
	 *            The expected type of token. Must not be <code>null</code>.
	 * @return The next token in the stream, which also matches the given class, or <code>null</code> if the next token
	 *         in the stream does not match the class.
	 */
	...
	/**
	 * Throws a {@link ChronoDBQuerySyntaxException} stating that the given sequence of tokens was expected, but the
	 * next token in the stream was found.
	 *
	 * @param tokenStream
	 *            The token stream to parse. Must not be <code>null</code>. Must not be in use by another parser.
	 *
