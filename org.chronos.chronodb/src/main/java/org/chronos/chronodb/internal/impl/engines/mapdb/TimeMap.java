...
class TimeMap {
	 ...
	 * @param branchName
	 *            The name of the branch to change the "now" timestamp for. Must not be <code>null</code>. Must
	...
	public static void put(final MapDBTransaction tx, final String branchName, final long timestampNow) {
		...
		getMap(tx).put(branchName, timestampNow);
	}
	 ...
	 *            The transaction to work on. Must not be <code>null</code>, must be open.
	 * @param branchName
	 *            The name of the branch to remove the "now" timestamp for. Must not be <code>null</code>. May refer
	 ...
	 *            The transaction to work on. Must not be <code>null</code>, must be open.
	 * @param branchName
	 *            The name of the branch to get the "now" timestamp for. Must not be <code>null</code>. Must refer
	...
	public static long get(final MapDBTransaction tx, final String branchName) {
		...
		Long timestamp = getMap(tx).get(branchName);
	...
	/**
	 * Returns the branch-to-timestamp map for the given transaction.
	 *
	 * @param tx
	 *            The transaction to work on. Must not be <code>null</code>, must be open.
	 *
	 * @return The map from branch names to "now" timestamps. May be empty, but never <code>null</code>.
	 */
	private static Map<String, Long> getMap(final MapDBTransaction tx) {
		checkNotNull(tx, "Precondition violation - argument 'tx' must not be NULL!");
